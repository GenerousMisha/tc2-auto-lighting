...
UI.TextButton.MouseButton1Click:Connect(function()
	for i,model in selection:Get() do
		if i > 1 then task.spawn(warnPlayer, "Please select only one model.") return end
		if not model:IsA("Folder") then task.spawn(warnPlayer, "Not a valid Folder.") return end
		if not model:FindFirstChild("Settings") then task.spawn(warnPlayer, "Make sure its a valid TC2 Map!") return end
		if not model.Settings:FindFirstChild("Lighting") then task.spawn(warnPlayer, "Make sure its a valid TC2 Map!") return end
		if not model.Settings:FindFirstChild("Terrain") then task.spawn(warnPlayer, "Make sure its a valid TC2 Map!") return end
...
button.Click:Connect(function() interface.Enabled = not interface.Enabled end)
